## Code borrowed from Optimize-Planning-Heuristics-to-Rank (https://github.com/aicenter/Optimize-Planning-Heuristics-to-Rank/)
...
class AttentionAugmentation2D(Layer):
    ...
    def __init__(self, depth_k, depth_v, num_heads, dim, relative=False, **kwargs):
        """
    ...
    def call(self, inputs, **kwargs):
        ...
        q = self.split_heads_2d(q)#1,2,15,15,5
        ...
        #print(logits.shape)#1,2,225,225
        ...
        #print("attn_out",attn_out.shape)#1,2,225,5
        ...
        #print(attn_out.shape)#1,2,15,15,5
        ...
        #print(self._shape)#last channel is still 30
        attn_out.set_shape(self.compute_output_shape(self._shape))#check if the shapes are same.
        #print(attn_out.shape)
        return attn_out

    def compute_output_shape(self, input_shape):
        output_shape = list(input_shape)
        output_shape[self.axis] = self.depth_v#(1,15,15,30) -> (1,15,15,10)
    ...
    def split_heads_2d(self, ip):
        ...
        self._batch = batch
        self._height = self._dim
        self._width = self._dim

        ret_shape = K.stack([batch, height, width,  self.num_heads, channels // self.num_heads])
        split = K.reshape(ip, ret_shape)#1,15,15,2,5
        transpose_axes = (0, 3, 1, 2, 4)
        split = K.permute_dimensions(split, transpose_axes)
        #print(split.shape)#1,2,15,15,5
        return split
    ...
    def combine_heads_2d(self, inputs):#1,2,15,15,5
        ...
        a, b = shape[-2:]#2,5
        ret_shape = K.stack(shape[:-2] + [a * b])#1,15,15,10
        # [batch, height, width, depth_v]
...
def augmented_conv2d(ip, filters, kernel_size=(3, 3), strides=(1, 1),
                     depth_k=0.2, depth_v=0.2, num_heads=4, relative_encodings=True):
    """
...
#    from tensorflow.keras.layers import Input, Dense, Conv2D, concatenate, Lambda, Multiply, Softmax, GlobalMaxPooling2D, GlobalAveragePooling2D
#    from tensorflow.keras.models import Model
#    from tensorflow.keras.callbacks import ModelCheckpoint
#    from readtrainSet import get_data15, get_test_data, get_dataTest, get_val_data
#    from train_sok import to_categorical_tensor
#    from success_next_state import next_state
#    import numpy as np
#    import tensorflow as tf
#    import math
#    import copy
#    ip = Input(shape=(15, 15, 4))
#    z = Conv2D(30, (3, 3), padding = "same", activation='relu', name = 'conv1')(ip)
#    x = AttentionAugmentation2D(10,10,2,relative=False)(z)
    #print(x.shape)#1,15,15,10
